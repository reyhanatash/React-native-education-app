...
const PublicExam: (Props) => React$Node = (Props) =>{
...
//   const [PdfUrl,setPdfUrl]=useState(0)
  ...
  const ShowAnswerSheet=()=>{
      for(let i=1;i<=Props.params.items.fldNumberQuestions ;i=i+1){
          if(Number.length<30){
...
const finishedQuiz=()=>{
 ...
 if(Props.params.items.fldType===6){
      push(Props,Screens.DedicatedExam,"",{id:Props.params.ID})
    }else{
...
const source = {uri:Props.params.items.fldFileUrl,cache:true};
console.warn("Props.params.items.fldFileUrl",Props.params.items)
...
console.warn("exam detail",examdetail!==undefined && examdetail.data.data[0])
    return(
        <View>
         <View style={{alignSelf:"flex-start",marginLeft:width*0.050}}>
                      ...
                      <CountDown
                        ...
                        until={examdetail!==undefined &&Props.params.items.fldTestTime*60}
